@echo off 


:: Unity本地路径
set UnityPath="D:\Program Files\Unity 2021.3.23f1\Editor\Unity.exe"
set CDNRoot="http://192.168.10.69:8099/"
set LOCAL_IIS="D:\zhongshengIIS"


set ReleaseProject=%~dp0..\..\MirrorRelease
set DevProjectPath=%~dp0..\
set ProjectPath=%~dp0..\..\
set GameVersion=4.0.2
set ResVersion=0.0.1
set IsClearAB=true
set OutDir=OutWeixin
set ABGuid=f3d746456b124184cb4d97418cf21de5
set WeiXinGuid=5785ba466acf1a54280279c44f64990b
set HotFixAllGuid=8b764c24f2b4b4e49baa85cc4b6e53f0
set BuildType=1
set IsCloseGuide=false
set IsDevelopmentBuild=false
set AppId=wx51abd3071918ea98
set OutDirGameVersion=%ProjectPath%%OutDir%\%GameVersion%

:: goto copyAsset

::判断Unity是否运行中
TASKLIST /V /S localhost /U %username%>tmp_process_list.txt
TYPE tmp_process_list.txt |FIND "Unity.exe"

IF ERRORLEVEL 0 (GOTO UNITY_IS_RUNNING)
ELSE (GOTO START_UNITY)


:: 新建/更新工程
call %~dp0新建Release工程.bat noPause


:: 关闭Unity工程
:UNITY_IS_RUNNING
::杀掉Unity
TASKKILL /F /IM Unity.exe
::停1秒
PING 127.0.0.1 -n 1 >NUL

echo ----------------------------------------------------
echo DevProjectPath %DevProjectPath%
echo ReleaseProject %ReleaseProject%
echo ----------------------------------------------------

:: 删除文件
:: echo Begin Delete Files

:: set StreamingResourcesPath=%DevProjectPath%Assets\StreamingAssets\StreamingResources\
:: rd /s /q %StreamingResourcesPath%
:: mkdir %StreamingResourcesPath%

:: rd /s /q %OutDirGameVersion%
:: mkdir %OutDirGameVersion%

:: goto endDo

:: 整包代码包处理

echo Begin Build HotFixAll Log:%ProjectPath%\HotFixAll.log

call %UnityPath% -quit ^
-batchmode ^
-projectPath "%ReleaseProject%" ^
-executeMethod TaskManagement.TaskEditorWindow.JenkinsStart ^
-logFile "%ProjectPath%\HotFixAll.log" ^
guid=%HotFixAllGuid% ^


:: AB包打包

echo Begin Build AB Log:%ProjectPath%\ABAllOutput.log

call %UnityPath% -quit ^
-batchmode ^
-projectPath %DevProjectPath% ^
-executeMethod TaskManagement.TaskEditorWindow.JenkinsStart ^
-logFile "%ProjectPath%\ABOutput.log" ^
guid=%ABGuid% ^
projectPath=%ProjectPath% ^
gameVersion=%GameVersion% ^
resVersion=%ResVersion% ^
isClearAB=%IsClearAB% ^
outDir=%OutDir% ^

::停1秒
PING 127.0.0.1 -n 1 >NUL

:: goto endDo

echo Begin Build Wexin Log:%ProjectPath%\WeiXinOutput.log

:: 微信出包
call %UnityPath% -quit ^
-batchmode ^
-projectPath %ReleaseProject% ^
-executeMethod TaskManagement.TaskEditorWindow.JenkinsStart ^
-logFile "%ProjectPath%\WeiXinOutput.log" ^
guid=%WeiXinGuid% ^
projectPath=%ProjectPath% ^
gameVersion=%GameVersion% ^
resVersion=%ResVersion% ^
isClearAB=%IsClearAB% ^
outDir=%OutDir% ^
cdnRoot=%CDNRoot% ^
buildType=%BuildType% ^
isCloseGuide=%IsCloseGuide% ^
isDevelopmentBuild=%IsDevelopmentBuild% ^
appId=%AppId% ^

::停1秒
PING 127.0.0.1 -n 1 >NUL

:copyAsset
echo "Begin Copy AB File"
:: 拷ab文件

set binPath=%ProjectPath%%OutDir%\%GameVersion%\Out\webgl
echo %binPath%
for /r %binPath% %%a in (*.bin.txt) do copy %%a %LOCAL_IIS%
xcopy %OutDirGameVersion%\%ResVersion%\%ResVersion%\ %LOCAL_IIS%\%ResVersion% /s/e/i/y/q
xcopy %OutDirGameVersion%\%ResVersion%\StreamingAssets\ %LOCAL_IIS%\StreamingAssets /s/e/i/y/q
xcopy %OutDirGameVersion%\%ResVersion%\Assets\ %LOCAL_IIS%\Assets /s/e/i/y/q

:endDo

pause