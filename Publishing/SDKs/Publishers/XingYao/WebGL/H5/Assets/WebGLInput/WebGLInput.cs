...
namespace WebGLSupport
{
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
        ...
        public static void SetSdkAction()
        {
            SDKManager.DoAwakeAction(() =>
                {
                    Debug.Log("WebGLInput DoAwakeAction");
                    SDKManager.Keyboard.OnKeyboardCreated = (go) =>
                    {
                        WebGLInput input = go.AddComponent<WebGLInput>();
                        input.enabled = true;
                        input.showHtmlElement = false;
                        Debug.Log("WebGLInput Created");
                    };
                }
            );
        }
        ...
        {
#if H5_NGUI
            if (GetComponent<UIInput>()) return new WrappedNGUIInputField(GetComponent<UIInput>());
#endif
